...
This guide may look overwhelming to some but as always I try to guide you thorugh every step of the way, in as much detail as possible so that anyone can follow my guides. If you don't already have [Notepad++](#) installed, I highly recomend it! This guide assumes you do. If you don't just use any text editor.
...
Now that you've edited the `b0000_defines` file, you can copy the modifed file back to the SNES using the FTP client.
...
To run the `sfc2sfrom.py` script you can use the included `.bat` file script I wrote. This will convert `.smc` and `.sfc` ROMS into the required `.sfrom` format (This is only required when using the native emulator `canoe`. You do not need to convert ROMS if you're using RetroArch).
...
Each ROM file requires (2) images. The main boxart image `CLV-P-XXXXX.png` `228x186`, and the thumbnail image `CLV-P-XXXXX_small.png` `40x32`. Create a folder for each game using the same naming scheme `CLV-P-XXXXX`, where (XXXXX) is a uniquie nameID using only Numbers and Capital letters. *Example: `CLV-P-DNA64`* 

I've written a script to help you with filenames, you can download that [HERE](#) if you like.
...
#### Transfering ROMS to the SNES Classic.
W.I.P

Launch Hakchi if it's not already running and select `Tools` from the menu bar at the top of the hakchi window and ensure that there is a check-mark next to “FTP server on `ftp://root:clover@127.0.0.1:1021`”. 
Leave Hakchi open.
...
If you get a C7, C8 or other error simply delete the last game you copied over from the system and power the system off. When you turn it back on you shouldn't get an error. If you do and you don't want to troubleshoot it, you can just flash the original kernel you dumped earlier back to the console using hakchi.

Special Thanks to **[ClusterM](http://clusterrr.com/)** and **[MadMonkey1907](https://www.reddit.com/user/madmonkey1907)** for creating hakchi and devoting so much of thier free time to the project and **[Valter](http://gbatemp.net/members/valter.381712/)** for his Python ROM Scripts.
